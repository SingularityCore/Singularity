...
void RARunnable::run()
{
    ...
    {
        sLog->outError("Singularity RA can not bind to port %d on %s", raport, stringip.c_str());
        return;
    }

    sLog->outString("Starting Singularity RA on port %d on %s", raport, stringip.c_str());
    ...
    sLog->outStaticDebug("Singularity RA thread exiting");
}
